...
from ppd_population import initial_disk_mass
...
def load_rnd_state_files(restart, chknum, refresh_rand_seed_on_restart):
    ...
    if (restart and not refresh_rand_seed_on_restart):
            ...
            if (num_sinks > 0): old_sink_tags = hydro.get_particle_tags(list(range(1,num_sinks+1)))
...
def make_stars_from_sinks(hydro, min_imf_mass):
    ...
    sink_tags = hydro.get_particle_tags(list(range(1,num_sinks+1)))
    sink_tags.sort()
...
def make_stars_from_sinks2(hydro, min_imf_mass, max_imf_mass, sample_imf_mass=10000 | units.MSun,
    ...
    for s,sink_mass in enumerate(sink_masses.value_in(units.MSun)):
        ...
        csum = np.cumsum(all_masses[sink_tags[s]] + \
            initial_disk_mass(
                all_masses[sink_tags[s]]|units.MSun).value_in(units.MSun))
        ...
        if nnew > 0:
            ...
            sink_mass = sink_mass - np.sum(spawn_masses) - initial_disk_mass(
                spawn_masses|units.MSun).value_in(units.MSun)
...
def update_roots_from_leaves(mult_grav, grav):
    ...
    '''
    for root, tree in mult_grav.root_to_tree.items():
...
stars, mult_grav, grav, _, _ = initialize_gravity_codes(
...
ppd_code = PPD_population(number_of_workers=, grid_hydro=hydro)
ppd_to_stars, stars_to_ppd = ppd_code.setup_self_channels(stars)
# due to the extra mass disks add, channels must be defined differently
grav_to_stars, stars_to_grav = ppd_code.setup_gravity_channels(stars, grav)
print ("PPD code initialized")
...
################    Not currently working for some reason!!!! (At least in Hermite) #########################
        ...
        ### Global timer for one loop
            ...
            #    pool.waitall()
                ...
                # ppds are evolved interwoven with gravity
                print ("Calling ppd code for first half step")
                stars_to_ppd.copy()
                ppd_code.evolve_model( t - dt/2. )
                ppd_to_stars.copy()
                ...
                # ppds are evolved interwoven with gravity
                print ("Calling ppd code for second half step")
                stars_to_ppd.copy()
                ppd_code.evolve_model( hydro_time )
                ppd_to_stars.copy()
            ...
            ### Check: Are the particles in sync across the two codes?
                ...
                if (tags_keys[:,0].all() == hydro.get_particle_tags(list(range(1,num_particles+1))).all() and
                    ...
                    print("hydro tags")
                    print(hydro.get_particle_tags(list(range(1,num_particles+1))))
                    print("tags tags")
                    print(tags_keys[:,0])
                    sys.stdout.flush()
                    sys.exit()
