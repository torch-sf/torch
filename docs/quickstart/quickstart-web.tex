...
If you used a local conda environment following this guide, you'll probably
need to tell \texttt{configure} where GMP, MPFR, and GSL reside.
Often, you'll have to tell \texttt{configure} where HDF5 and FFTW live, too.
...
The STDOUT from a successful configure build should include (valid for AMUSE
v12.0.0, but changed by AMUSE commit \href{https://github.com/amusecode/amuse/commit/9a3b134eeee665b838cb84ff45b85df0887a77bb}{9a3b134e})
\begin{lstlisting}[language=bash]
...
The file \texttt{bridge\_multiples.py} is the heart of the simulation.  It
performs the split time-evolution with both FLASH and the n-body integrator
ph4, handles stellar evolution, and lots more.
...
and \texttt{bridge\_multiples.py}.  These two files specify most of the
...
A normal Torch run initializes from a "cubefile" like \texttt{cube128} or one generated from \texttt{turb-sphere.py}. These initial conditions are currently restricted to spherical clouds with a turbulent velocity distribution and gaussian density profile or something similar. While such ICs make for easy first steps for users to setup a new Torch simulation, ideal ICs would come from systems that evolved self-consistently in galactic environments. Unfortunately, Torch on its own cannot currently model such systems (and the grid nature of FLASH precludes it to have significant angular momentum loss and so should not be used to model rotating systems like galaxies). 
...
To turn \texttt{VorAMR} on, switches must be flipped in two separate places: in the FLASH source which is installed into 
\$FLASH\_HOME and the Torch source.

1. In \texttt{torch/src/flash/source/Simulation/SimulationMain/Cube/pt\_initVoronoiPositions.F90}, uncomment \texttt{\#define VORAMR}.

2. In \texttt{torch/torch\_user.py user\_parameters()} set the following: \texttt{p['with\_voramr'] = True} and provide the file names as required.

The \texttt{source\_file} is the raw output data that you want to interpolate into Torch. \texttt{convert\_file} will apply the 
routines in \texttt{hdf5\_convert.py} to the source data (unit conversions, centering, and velocity normalization) and output 
a simplified hdf5 file that mimics the AREPO data structure. \texttt{pickle\_kdtree} will pickle the k-dimensional tree generated 
by \texttt{kdtree.py} for quick use on successive runs.

Once VorAMR is turned on, install torch \texttt{. ./install.sh}.
...
Change the `lrefine\_max` value in flash.par to be whatever level you wish the grid to reach at its highest refinement regions.

Uncomment the VorAMR parameters in flash.par. Be sure to re-comment these if you later starting a "normal" Torch run.

\begin{lstlisting}[language=bash]
refine_on_particle_count=.true.
min_particles_per_blk=4096
max_particles_per_blk=4096 
pt_maxPerProc = 10000000
\end{lstlisting}
...
Finally, run Torch with only 1 processor assigned to \texttt{flash\_worker} (if running default worker partitions, this would be 6 total processors: 1 driver, 2 \texttt{ph4}, 1 \texttt{SeBa}, 1 \texttt{multiples}, 1 \texttt{FLASH}).
...
The refactored version of Torch (on git branch \texttt{refactor}) uses the latest AMUSE commit on branch \texttt{master} which requires Python 3.X as well as FLASH version 4.6.2. The setup for the refactored version of Torch on Cartesius is largely the same as the \texttt{master} branch as described in this document but with a dew caveats.
...
After gaining access to Snellius, download Torch and \texttt{AMUSE} via github/bitbucket and \texttt{FLASH} via the Rochester University portal. 

The following is a \texttt{Bash} environment file for modules and paths. Run these commands (or something similar) before installing Torch or compiling \texttt{FLASH} or \texttt{AMUSE}.
\begin{lstlisting}[language=bash]
#Load modules
...
Then, build your conda environment. The mpi4py install is very particular about the system environment in which it is installed, if you're running into issues later, reinstalling mpi4py or double checking paths is a good place to start.
\begin{lstlisting}[language=bash]
mkdir {your/conda_env/directory} 
...
A few changes need to be made inside of the Torch and \texttt{FLASH} architecture to bring up to date with Snellius' compiler and Python version.
\begin{lstlisting}[language=bash]
cd torch
#in src/flash/source/Particles/ParticlesMain/ray_pe/MultiSourceSimple/HEALPixModule.F90
#line 209 & 250:
iand(int(x),y) #i.e. replace the first argument of iand with int(argument)
./install.sh

emacs $FLASH_DIR/bin/setup.py
#change sys.setcheckinterval() to sys.setswitchinterval()
cd $FLASH_DIR/sites
cp -r cartesius.surfsara.nl snellius.surf.nl
#in $FLASH_DIR/sites/snellius.surf.nl/Makefile.h
#add -fallow-argument-mismatch to FFLAGS_OPT
#in $FLASH_DIR/sites/Aliases
add:
snellius.surf.nl int1.local.snellius.surf.nl
snellius.surf.nl int2.local.snellius.surf.nl
\end{lstlisting}
...
\begin{lstlisting}[language=bash]
cd amuse
./configure --with-fftw=$MODLOC/FFTW/3.3.9-gompi-2021a --with-hdf5=$MODLOC/HDF5/1.10.7-gompi-2021a/bin/h5c++ --with-gmp=/$CONDA_ENV --with-mpfr=$CONDA_ENV --with-gsl-prefix=/home/bpolak/TORCH/env/amuse_env 
#note: some users found that "./configure" what sufficient without any of the --with flags.
