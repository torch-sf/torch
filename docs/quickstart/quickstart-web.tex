...
\end{lstlisting}
If you have a previously installed version of AMUSE, make sure that it is more recent than \textbf{Feb 11, 2025}.
...
You will also need to add the following flags to \texttt{FFLAGS\_OPT}:
\texttt{-I\$HDF5\_INC -L\$HDF5\_LIB -lhdf5\_fortran}, with the linkers pointing
to your HDF5 install.
...
You will need to add the following flags to \texttt{FCFLAGS} in both \texttt{config.mk}
and \texttt{src/amuse/community/flash/Makefile}:
\texttt{-I\$HDF5\_INC -L\$HDF5\_LIB -lhdf5\_fortran}, with the linkers pointing
to your HDF5 install.
...
\end{lstlisting}
in \texttt{torch\_user.py}. \texttt{PeTar} accepts the user parameter \texttt{r\_out} which is a boundary radius between the tree gravity and direct N-body and SDAR. The default is the standard value used in cluster simulations, but the user should test this value and modify it for their particular use case. For a thorough understanding of the parameters used in \texttt{PeTar}, users should familiarize themselves with the \href{https://github.com/lwang-astro/PeTar}{README} of the \texttt{PeTar} github and the \texttt{PeTar} \href{https://ui.adsabs.harvard.edu/abs/2020MNRAS.497..536W/abstract}{methods paper}.
...
This setup guide is credited to Brooke Polak. \textbf{CARTESIUS IS NOW DISCONTINUED} - We leave the build instructions here as many of the steps are similar to other systems.
...
You can then configure FLASH, by following the instructions below. In \texttt{\$FLASH_DIR/sites}, run:
\begin{lstlisting}
cp -r sites/cartesius.surfsara.nl sites/stampede3.tacc.utexas.edu
\end{lstlisting}
Then in \texttt{\$FLASH_DIR/sites/stampede3.tacc.utexas.edu/Makefile.h}, set:
\begin{lstlisting}
MPI_PATH   = ${MPI_HOME}
HDF5_PATH   = ${HDF5_HOME}
