...
On l. 229, a CFL of 0.3 is hardcoded in the dt_wind routine. Also see l. 271, there is a pre-factor of 0.3 hardcoded in the dt_min_local calculation. Note that on l. 267, cfl_radPressure is used, which is a runtime parameter. If we try to reduce the CFL is hot zone or raise it to speed up the code, this change will not propagate to the routine. 
