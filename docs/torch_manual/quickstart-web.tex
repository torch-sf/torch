...
%Torch may work with parallel HDF5 and/or Python 3.x, but has not been tested as
%much with those packages.
\begin{itemize}
%\item \texttt{FLASH4.5} repo commit \texttt{68dfed1},
%    \url{https://bitbucket.org/joshuawall/FLASH4.5}
\item FLASH 4.5 (support for 4.6.2 is coming soon): \url{http://flash.uchicago.edu/site/flashcode}
\item AMUSE: \url{https://github.com/amusecode/amuse}
\item Torch: \url{https://bitbucket.org/torch-sf/torch}
\item Conda / Python 2.7
\item Python 3.X (for Torch \texttt{refactor} only)
\item MPI
\item HDF5
\item git, wget
\end{itemize}
...
% Build FLASH 4.5
% ---------------
\subsection{Build FLASH 4.5 (Josh thesis Appendix A.2 + FLASH docs)}

Let's \texttt{cd} to the FLASH4.5 directory and setup a turbulent sphere
collapse problem.

First, create a \texttt{Makefile.h} in \texttt{FLASH4.5/sites/mysitedir}.
...
%Alternatively, you can add your machine to \texttt{FLASH4.5/sites/Aliases} in
...
%\texttt{FLASH4.5/source/Simulation/SimulationMain/Cube/Simulation\_initBlock.F90}.

Torch comes packaged with a few simulations in
\texttt{FLASH4.5/source/Simulation/SimulationMain/}
\begin{verbatim}
    Cube
    EnergyInjection
    StratBox
...
\texttt{FLASH4.5/bin/setup\_shortcuts.txt}.
...
* MAXBLOCKS * (8 bytes)
\end{verbatim}
        where the variables are defined in \texttt{FLASH4.5/object/Flash.h}.
