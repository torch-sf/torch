...
def make_data_cube(Msph, Rsph, box, n0, Tsph, T_amb, musph, mu_amb, vir_rat,
    ...
    if (write_data):
        ...
        print("making zero'd data.")
        rho_zerod = np.zeros_like(rho_arr)
        p_zerod = np.zeros_like(p_arr)
        velx_zerod = np.zeros_like(velx)
        vely_zerod = np.zeros_like(vely)
        velz_zerod = np.zeros_like(velz)
        pot_zerod = np.zeros_like(pot_arr)
        ...
        for i in range(NCD[0]):
            for j in range(NCD[1]):
                for k in range(NCD[2]):
                   savetxt(f, array((i,j,k, rho_zerod[i,j,k], p_zerod[i,j,k]\
                    , velx_zerod[i,j,k], vely_zerod[i,j,k], velz_zerod[i,j,k], pot_zerod[i,j,k])).reshape(1,9), \
...
if (infile is None):
    ...
    Msph, Rsph, box, vir_rat, n0, Tsph, T_amb, kmin, kmax, Eslp, Bmag = np.loadtxt(
            infile, dtype=np.float, skiprows=2, usecols=list(range(11)), unpack=True)
