...
!NOTE: This subroutine APPENDS to the end of the file "fname". CB not anymore it don't
    ...
    select case (forma)
      case('u','U')
!       open(unit=17,file=trim(fname),status='UNKNOWN',form='UNFORMATTED', &
!            position='APPEND')
       open(unit=17,file=trim(fname),status='replace',form='UNFORMATTED')
       write(17)mti
       write(17)mt

      case default
!       open(unit=17,file=trim(fname),status='UNKNOWN',form='FORMATTED', &
!            position='APPEND')
       open(unit=17,file=trim(fname),status='replace',form='FORMATTED')
       write(17,*)mti
       write(17,*)mt
...
!         state information will be read in formatted form.
  subroutine mtgetf( fname, forma )
    ...
    select case (forma)
      case('u','U')
       open(unit=17,file=trim(fname),status='OLD',form='UNFORMATTED')
       read(17)mti
       read(17)mt

      case default
       open(unit=17,file=trim(fname),status='OLD',form='FORMATTED')
       read(17,*)mti
       read(17,*)mt
