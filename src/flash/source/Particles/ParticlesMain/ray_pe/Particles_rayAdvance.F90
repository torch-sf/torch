...
#include "Flash.h"
  ...
  use Particles_rayData, only : useRadTransfer
  use pt_rayInterface
...
include "Flash_mpi.h" 
  ...
  if(.not. useRadTransfer) return

#ifdef SINK_PART_TYPE
! If this is the first call and sinks are in, then we should
! sort them in case any were added using Simulation_init*
! since that occurs *before* grid refinement. - JW
  if (first_call) then
        !print*, "About to sort sinks."
        call Particles_moveAndSort(.true.)
        first_call = .false.
        !print*, "Done sorting sinks."
!     end if   
  end if
#endif

 ! I think this is needed but not 100% on this one 
  call gr_ensureValidNeighborInfo(10)

! raytracing is not really dependent on rest of the particle data structure
! allocate memory for local rays, plus buffers for communication 

  call pt_initRays()
! generate new rays from sources
! this happens every hydro half time step
! face sources here
  call pt_generateRaysFace(dt)

! pointy sources here
  call pt_generateRaysPoint(dt)

! calls init routines for async communication, otherwise stub
  call pt_startComm()

! actual raytracing
  call pt_advanceRaysFace(dt)

! "advanced"
  call pt_advanceRaysPoint(dt)

! clean memory, spring fresh
  call pt_finalizeRays()
  return
