...
!! modified to include PE photons by J. Wall 2017.
...
#ifdef PE_HEAT
! # of PE photons.
    sourcedata(NPEP_PART_PROP)  = particles(NPEP_PART_PROP, sourceStr)
! average energy of PE photons.
    sourcedata(EPEP_PART_PROP)  = particles(EPEP_PART_PROP, sourceStr)
! cross section of dust to PE photons.
    sourcedata(SPEP_PART_PROP)  = particles(SPEP_PART_PROP, sourceStr)
#endif
! next source
    i = i + 1
  else
