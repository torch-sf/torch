...
!! Modified to include sink particles. J. Wall 2015
...
! This shouldn't be defined this way... -JW
!#ifdef ACTIVE
! instead it should be... -JW
...
! this should take the particle as argument to fully decouple from number of particle properties
  ...
  do while (i .ge. 0)
    ...
    !print*, "[pt_generateRaysPoint]: ionizingN=, i=", ionizingN, i, dr_globalMe
...
! int*4 might not be enough, save as real?
      ...
      raysIntProp(itype, p)  = ion_photon
...
!   for rays this is the direction of the ray
    ...
    photoElecN = sourcedata(NPEP_PART_PROP)
! If there are no PE photons, we aren't going to make any rays.
! Note this gives me a switch to turn off radiation for stars less than 8 MSun. -JW
  if (photoElecN .gt. 0.0) then

! Number of photoelectric photons at surface of star per HEALPix pixel in this timestep
    photoElecN   = dt*photoElecN/(n_photons+1)
...
!   column, distance from source, healpix level and number
      raysRealprop(inion,p)  = photoElecN
      raysRealProp(ieion,p)  = sourcedata(EPEP_PART_PROP)
      raysRealProp(isigh,p)  = sourcedata(SPEP_PART_PROP)
      raysRealProp(irad,p)   = 0d0
      raysIntProp(ihlev, p)  = nside
...
!   for rays this is the direction of the ray
      ...
      raysRealprop(ivelz,p)  = dir(3)
    enddo
  end if ! If photoelectric photons - JW
...
! Okay, how bout some sinks then? - JDub
...
! loop over sources
  i = 0
! this should take the particle as argument to fully decouple from number of particle properties
  call pt_assignRaySink(i, sourcedata)
  
  do while (i .ge. 0)
    ionizingN   = sourcedata(NION_PART_PROP)
    
! If there are no ionizing photons, we aren't going to make any rays.
! Note this gives me a switch to turn off radiation for stars less than 8 MSun. -JW
  if (ionizingN .gt. 0.0) then
