...
#include "Particles.h"
  ...
  integer, dimension(2*MDIM) :: order
	real 											 :: dens, mflx, temp, time,freeze!, SBid !, stepsize,
	real 											 :: x, y, z
	real 											 :: xpos, ypos, zpos
	integer                    :: xID, yID, zID
...
!  real, allocatable,dimension(:,:,:) :: totXFlx
  ...
  integer, parameter :: xhalf=NXB/2, yhalf=NYB/2, zhalf=NZB/2
...
! from 5->1,2;  6->3,4; 7->5,6; 8->7,8 for ge 5
			  if     (xID .le. xhalf .and. yID .le. yhalf .and. zID .le. zhalf ) then
...
#endif
				else if(xID .ge. xhalf+1 .and. yID .le. yhalf .and. zID .le. zhalf ) then
...
! map x 	
				  ...
				  print*,'mapped ids',xID,yID,zID
#endif
		    else if(xID .ge. xhalf+1 .and. yID .ge. yhalf+1 .and. zID .le. zhalf ) then
! x 5..8 to 1...8, y 5..8 to 1...8, z 1..4 to 1...8
					...
					print*,'mapped ids',xID,yID,zID
#endif
				else if(xID .le. xhalf .and. yID .ge. yhalf+1 .and. zID .le. zhalf ) then
! x 1..4 to 1...8, y 5..8 to 1...8, z 1..4 to 1...8
					...
					print*,'mapped ids',xID,yID,zID
#endif
				else if(xID .le. xhalf .and. yID .le. yhalf .and. zID .ge. zhalf ) then
! x 1..4 to 1...8, y 1..4 to 1...8, z 5..8 to 1...8
					...
					print*,'mapped ids',xID,yID,zID
#endif
				else if(xID .ge. xhalf+1 .and. yID .le. yhalf .and. zID .ge. zhalf+1 ) then
! x 5..8 to 1...8, y 1..4 to 1...8, z 5..8 to 1...8
					...
					print*,'mapped ids',xID,yID,zID
#endif
				else if(xID .ge. xhalf+1 .and. yID .ge. yhalf+1 .and. zID .ge. zhalf+1 ) then
! x 5..8 to 1...8, y 5..8 to 1...8, z 5..8 to 1...8
					...
					print*,'mapped ids',xID,yID,zID
#endif
				else if(xID .le. xhalf .and. yID .ge. yhalf+1 .and. zID .ge. zhalf+1 ) then
...
! check if particle has to be frozen out or not 
! criteria for froziness
		freeze = particles(SBID_PART_PROP,i)

		if(freeze .lt. 0) then
		  temp = blockData(TEMP_VAR,xID+xIoffset,yID+yIoffset,zID+zIoffset)
			time = particles(TCRT_PART_PROP,i)
			if(temp .lt. ej_TempUnfreeze) then ! temp threshold
				! forever unfreeze
				particles(SBID_PART_PROP,i) = 1d0
			else if (dr_simTime-time .gt. ej_TimeUnfreeze) then ! time 
				! forever unfreeze
				particles(SBID_PART_PROP,i) = 1d0
			else if (ej_shockUnfreeze) then
				shock = blockData(SHK_VAR,xID+xIoffset,yID+yIoffset,zID+zIoffset)
				if(shock .gt. 0d0) then 
					particles(SBID_PART_PROP,i) = 1d0
				else
					cycle
				endif
...
! also weak tests, could be out of lower or upper bounds
							  if     (yID .le. xhalf .and. zID .le. zhalf) then ! 1 
					        newBID    = negh(1,1)! 
									lowbx = bnd_box(LOW,IAXIS,blockID)
									lowby = bnd_box(LOW,JAXIS,blockID)
									lowbz = bnd_box(LOW,KAXIS,blockID)
							  else if(yID .ge. xhalf+1 .and. zID .le. zhalf) then ! 2 
					        newBID    = negh(1,2)
									lowbx = bnd_box(LOW,IAXIS,blockID)
									lowby = bnd_box(LOW,JAXIS,blockID) + (bnd_box(HIGH,JAXIS,blockID)- bnd_box(LOW,JAXIS,blockID))/2d0
									lowbz = bnd_box(LOW,KAXIS,blockID)
							  else if(yID .le. yhalf .and. zID .ge. zhalf+1) then ! 3
					        newBID    = negh(1,3)
									lowbx = bnd_box(LOW,IAXIS,blockID)
									lowby = bnd_box(LOW,JAXIS,blockID)
									lowbz = bnd_box(LOW,KAXIS,blockID) + (bnd_box(HIGH,KAXIS,blockID)- bnd_box(LOW,KAXIS,blockID))/2d0
							  else if(yID .ge. yhalf+1 .and. zID .ge. zhalf+1) then ! 4
					        newBID    = negh(1,4)
									lowbx = bnd_box(LOW,IAXIS,blockID)
									lowby = bnd_box(LOW,JAXIS,blockID) + (bnd_box(HIGH,JAXIS,blockID)- bnd_box(LOW,JAXIS,blockID))/2d0
									lowbz = bnd_box(LOW,KAXIS,blockID) + (bnd_box(HIGH,KAXIS,blockID)- bnd_box(LOW,KAXIS,blockID))/2d0
...
! z low:  1..4; z high: 2..8
! also weak tests, could be out of lower or upper bounds
							  if     (xID .le. xhalf .and. zID .le. zhalf) then ! 1
					        newBID    = negh(1,1)
									lowby = bnd_box(LOW,JAXIS,blockID)
									lowbx = bnd_box(LOW,IAXIS,blockID)
									lowbz = bnd_box(LOW,KAXIS,blockID)
							  else if(xID .ge. xhalf+1 .and. zID .le. zhalf) then ! 2 
					        newBID    = negh(1,2)
									lowby = bnd_box(LOW,JAXIS,blockID)
									lowbx = bnd_box(LOW,IAXIS,blockID) + (bnd_box(HIGH,IAXIS,blockID)- bnd_box(LOW,IAXIS,blockID))/2d0
									lowbz = bnd_box(LOW,KAXIS,blockID)
							  else if(xID .le. xhalf .and. zID .ge. zhalf+1) then ! 3
					        newBID    = negh(1,3)
									lowby = bnd_box(LOW,JAXIS,blockID)
									lowbx = bnd_box(LOW,IAXIS,blockID)
									lowbz = bnd_box(LOW,KAXIS,blockID) + (bnd_box(HIGH,KAXIS,blockID)- bnd_box(LOW,KAXIS,blockID))/2d0
							  else if(xID .ge. xhalf+1 .and. zID .ge. zhalf+1) then ! 4
					        newBID    = negh(1,4)
									lowby = bnd_box(LOW,JAXIS,blockID)
									lowbx = bnd_box(LOW,IAXIS,blockID) + (bnd_box(HIGH,IAXIS,blockID)- bnd_box(LOW,IAXIS,blockID))/2d0
									lowbz = bnd_box(LOW,KAXIS,blockID) + (bnd_box(HIGH,KAXIS,blockID)- bnd_box(LOW,KAXIS,blockID))/2d0
...
! y low:  1..4; y high: 2..8
! also weak tests, could be out of lower or upper bounds
							  if     (xID .le. xhalf .and. yID .le. yhalf) then ! 1 
					        newBID    = negh(1,1)
									lowbz = bnd_box(LOW,KAXIS,blockID)
									lowbx = bnd_box(LOW,IAXIS,blockID)
									lowby = bnd_box(LOW,JAXIS,blockID)
							  else if(xID .ge. xhalf+1 .and. yID .le. yhalf) then ! 2 
					        newBID    = negh(1,2)
									lowbz = bnd_box(LOW,KAXIS,blockID)
									lowbx = bnd_box(LOW,IAXIS,blockID) + (bnd_box(HIGH,IAXIS,blockID)- bnd_box(LOW,IAXIS,blockID))/2d0
									lowby = bnd_box(LOW,JAXIS,blockID)
							  else if(xID .le. xhalf .and. yID .ge. yhalf+1) then ! 3
					        newBID    = negh(1,3)
									lowbz = bnd_box(LOW,KAXIS,blockID)
									lowbx = bnd_box(LOW,IAXIS,blockID)
									lowby = bnd_box(LOW,JAXIS,blockID) + (bnd_box(HIGH,JAXIS,blockID)- bnd_box(LOW,JAXIS,blockID))/2d0
							  else if(xID .ge. xhalf+1 .and. yID .ge. yhalf+1) then ! 4
					        newBID    = negh(1,4)
									lowbz = bnd_box(LOW,KAXIS,blockID)
									lowbx = bnd_box(LOW,IAXIS,blockID) + (bnd_box(HIGH,IAXIS,blockID)- bnd_box(LOW,IAXIS,blockID))/2d0
									lowby = bnd_box(LOW,JAXIS,blockID) + (bnd_box(HIGH,JAXIS,blockID)- bnd_box(LOW,JAXIS,blockID))/2d0
...
! move to local coordinate system
								...
								if(abs(xyzID(1) - xID) .ne. NXB-1 ) then
  								print*,xyzID(1),'->',xID,'x-',i,pt_xmax,pt_xmin 
									print*,'broky broke',i
									stop
								endif
...
#endif
								if(abs(xyzID(2) - yID) .ne. NYB-1 ) then 
   								print*,xyzID(2),'->',yID,'y-',i,pt_ymax,pt_ymin
									print*,'broky broke',i
									stop
								endif
...
! neg. z
					    case (3)
							  z    = z - gridspace(3)/2d0
								...
								if(abs(xyzID(3) - zID) .ne. NZB-1 ) then 
  								print*,xyzID(3),'->',zID,'z-',i,pt_zmax,pt_zmin
									print*,'broky broke',i
									stop
								endif
...
!//////////////// pos. direction
! this also selects the face
				else if( direction .gt. 0 .and. xyzID(dir) .ge. NXB) then
...
! also weak tests, could be out of lower or upper bounds
							  if     (yID .le. yhalf .and. zID .le. zhalf) then ! 1 
					        newBID    = negh(1,1)! 
									lowbx = bnd_box(HIGH,IAXIS,blockID)
									lowby = bnd_box(LOW,JAXIS,blockID)
									lowbz = bnd_box(LOW,KAXIS,blockID)
							  else if(yID .ge. yhalf+1 .and. zID .le. zhalf) then ! 2 
					        newBID    = negh(1,2)
									lowbx = bnd_box(HIGH,IAXIS,blockID)
									lowby = bnd_box(LOW,JAXIS,blockID) + (bnd_box(HIGH,JAXIS,blockID)- bnd_box(LOW,JAXIS,blockID))/2d0
									lowbz = bnd_box(LOW,KAXIS,blockID)
...
#ifdef DEBUG_MC
				if(xID .lt. 1 .or. yID .lt. 1 .or. zID .lt. 1 .or. xID .gt. NXB .or. yID .gt. NYB .or. zID .gt. NZB ) then
