...
!!		working on the SC (sub-grid cluster module)
!!
!!
!!***
...
subroutine Particles_sinkCreateAccrete(dt)
  ...
  use Logfile_interface, ONLY : Logfile_stamp
	...
	use SC_data, ONLY: sc_SF_delay, sc_SFE

!   use SB_Data, ONLY: sb_life, sn_cloudMassPerStar
!	Used to accrete tracer particles in the SNsink particles
!	use SN_data, only : he_useSNTracer
...
#include "GridParticles.h"
  ...
  logical			:: useSNsink
	...
	integer :: i, n, clock									! loop indexes, and random number indexes.
...
#define get_ppe(arg1) ((arg1) - get_pno(arg1)*65536)
  ...
  if (first_call) then
    ...
    call RuntimeParameters_get("useSNsink", useSNsink)
  ...
  do llb = 1, llnblocks
              ...
              if (comovingCellDens .gt. density_thresh_comoving) then
                 ...
                 if (np_found .gt. 0) then
                    ...
                    if (pno_to_accrete .gt. 0) then
					   ...
					   ! JC: Not yet ported the accretion of passive tracers
						! if(sb_accTracer .and. he_useSNTracer) then
					   ! 	call pt_findTracer(lb, ip, jp, kp, dx_block, blkLimits(LOW,IAXIS), blkLimits(LOW,JAXIS), blkLimits(LOW,KAXIS), &
					   ! 										 accTr(pno_to_accrete),sn_tracerAccFrac)
					   ! endif
   ...
   do lp = 1, localnp
      ...
      if (tot_mass(lp) .ne. 0.0) then
		 ...
		 if (particles_local(ipm,lp) .gt. 0.0) then ! There is some mass inside the sink.
		 	
			! If the  Particle formation + SF_delay < time < Particle formation + SF_delay + dt		
		 	! if(particles_local(ipt,lp) + 1.0e+12 .lt. time .and. particles_local(CLUSTER_POP_PART_PROP, lp) .eq. 0.0 ) then ! Is is time to sample the cluster ?
			if(particles_local(ipt,lp) + sc_SF_delay .lt. time .and. particles_local(CLUSTER_POP_PART_PROP, lp) .eq. 0.0 ) then ! Is is time to sample the cluster ?
				
				! Number of stellar populations in this sink
		 		particles_local(CLUSTER_POP_PART_PROP, lp) = 1.0
		 	
		 		! Cluster Creation time
		 		particles_local(CLUSTER_CRT_PART_PROP,lp) = time
		 	
		 		! Cluster Mass
		 		particles_local(CLUSTER_MASS_PART_PROP, lp) = particles_local(ipm, lp) * sc_SFE
		 		!particles_local(CLUSTER_MASS_PART_PROP, lp) = 1
		 		
		 		! Cluster random seed
				! Right now I'm using the particle tag as as the cluster seed.
				particles_local(CLUSTER_SEED_PART_PROP, lp ) = particles_local(iptag, lp)
				
				PRINT*,""
				PRINT*,"Hello, I'm a sink and I am about to sample the cluster."
				PRINT*,"Condition: particle_creation_time + sc_SF_delay > time " 
				PRINT*,"particle creation time, SF delay and current time: ", particles_local(ipt,lp), sc_SF_delay, time
				PRINT*,"Number of stellar populations in sink:   ", particles_local(CLUSTER_POP_PART_PROP, lp)
				PRINT*,"Creation time of the stellar population: ", particles_local(CLUSTER_CRT_PART_PROP,lp)
				PRINT*,"Stellar mass in the cluster:             ", particles_local(CLUSTER_MASS_PART_PROP, lp)
				PRINT*,"Random seed used to sample the cluster:  ", particles_local(CLUSTER_SEED_PART_PROP, lp )
				PRINT*,"Particle ID                              ", particles_local(iptag, lp)
				PRINT*,""
				
				! Write a file containing the cluster info.
								
				! I guess if there are two clusters created at the same time, trying to access the same file 
				! simultaneously, then I'm screwed.
				! When a cluster is created. I should Write a file saving the cluster ID. cluster index.
				! the associated sink particle and the basic cluster information.
				
				! First cluster sample.
				call SC_clusterSample(Mass, seed)
				
				! There should be another step when I call sample the cluster and evolve it.

				! I should also make a folder where the evolution of each cluster is saved.
