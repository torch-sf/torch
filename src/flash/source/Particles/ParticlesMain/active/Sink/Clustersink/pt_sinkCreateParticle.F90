...
! Unique ID of the super bubble
	particles_local(SBID_PART_PROP,pno)  = 0d0

! Creation Time of the Super bubble.
	particles_local(TCRT_PART_PROP,pno)  = 0d0

!/////////////// sub-grid stellar cluster model
!///////////////////////////////////////////////////

! JCIM - New properties of the cluster particle:

	! cluster creation time.
  	particles_local(CLUSTER_CRT_PART_PROP,pno) = 0d0

	! Random seed used to stochastically sample the cluster
	particles_local(CLUSTER_SEED_PART_PROP, pno)  = 0d0
	
	! Mass of the cluster at the moment I sample it
	particles_local(CLUSTER_MASS_PART_PROP, pno) = 0d0
	
	! Number of stellar populations inside a sink.
	! Right now it only works with one, but I could extend that later.
	! properties should be made arrays instead of a single value for multiple populations
	particles_local(CLUSTER_POP_PART_PROP, pno) = 0
