...
!!		Added the sub-grid stellar cluster module.
!!
!!***
...
#include "Particles.h"
  ...
  ! JCI -three new properties added for the SNsink particles (NSN, TSN, TCRT). SB module
  ! JCI -three new properties added for the SNsink particles (CLUSTER_SEED, CLUSTER_MASS, CLUSTER_CRT, CLUSTER_POP) sub-grid cluster module
  integer, parameter :: gather_nprops = 23
  integer, dimension(gather_nprops), save :: gather_propinds = &
    (/ integer :: TAG_PART_PROP, MASS_PART_PROP, ACCR_RATE_PART_PROP, &
                  ...
                  CREATION_TIME_PART_PROP, NSN_PART_PROP, TSN_PART_PROP, &
 				  TCRT_PART_PROP, CLUSTER_SEED_PART_PROP, CLUSTER_MASS_PART_PROP, &
 				  CLUSTER_CRT_PART_PROP, CLUSTER_POP_PART_PROP /)
  ...
  if (.not. header_written) then
     ...
     write(funit_evol,'(20(1X,A16))') '[00]part_tag', '[01]time', '[02]posx', '[03]posy', '[04]posz', &
									  ...
									  '[20]seed', '[21]cluster mass', '[22]cluster creation t', '[23]populations'
  ...
  do i = 1, localnpf
     ...
     write(funit_evol,'(1(1X,I16),25(1X,ES16.9))') &
!     write(funit_evol,'(1(1X,I16),18(1X,ES16.9))') &
          ...
          particles_global(CLUSTER_SEED_PART_PROP,i), &
          particles_global(CLUSTER_MASS_PART_PROP,i), &
          particles_global(CLUSTER_CRT_PART_PROP,i), &
          particles_global(CLUSTER_POP_PART_PROP,i)
