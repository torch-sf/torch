...
#define get_ppe(arg1) ((arg1) - get_pno(arg1)*65536)
  ...
  do llb = 1, llnblocks
   ...
   do lp = 1, localnp
	...
	! JC: check if there is enough mass in the sink particle to host a SN.

	if(time .lt. particles_local(ipt,lp)  + dt ) then
		print*,'new sink, mass',particles_local(ipm,lp),lp

! TODO: Given the SFdelay, take the mass inside the sink and sample a stellar population inside.

		if (particles_local(ipm,lp) .gt. 0.0) then ! Cluster particle
			particles_local(NSN_PART_PROP,lp)  = 1.0
			particles_local(TSN_PART_PROP,lp)  = 1.0e11 !8.5137e11 !1.0e12 ! Delay for the simulation I am running.
			particles_local(TCRT_PART_PROP,lp) = time
			
		end if   

!			!	  if there is enough mass in the Sink a cluster particle is created.
!			!	  if there is not enough mass to create a cluster, a single SN is created.
!			!	  if there is not enough mass to host a SN, no SN inside this particle.
!			! TODO: Link the subgrid model here to inform the sink particle of the stochasticity of the IMF sampling.	
!			if (particles_local(ipm,lp) .gt. sn_cloudMassPerStar) then ! Cluster particle
!				particles_local(NSN_PART_PROP,lp) = floor(particles_local(ipm,lp)/sn_cloudMassPerStar)
!				particles_local(TSN_PART_PROP,lp) = sb_life/(particles_local(NSN_PART_PROP,lp)+1)
!			else
!				if (particles_local(ipm,lp) .gt. 0.8*sn_cloudMassPerStar)then ! Single star
!					particles_local(NSN_PART_PROP,lp) = floor(particles_local(ipm,lp)/sn_cloudMassPerStar)
!					! Testing purposes only, blow up after 0.1 Myr of creation.
!					particles_local(TSN_PART_PROP,lp) = 0.1 * 3.15e13
!					! Default of 10 Myr for the SN to blow up
!					!particles_local(TSN_PART_PROP,lp) = 10 * 3.1556e13
!				else	! Not massive enough to be a SN
!					particles_local(NSN_PART_PROP,lp) = 0.0
!					particles_local(TSN_PART_PROP,lp) = 0.0
!				endif ! SN or not SN
!			endif	! Cluster or single SN.

		print*,'******* new sink, number of SN in sink, Time between SN', particles_local(NSN_PART_PROP,lp),particles_local(TSN_PART_PROP,lp)
	end if
		!particles_local(TCRT_PART_PROP,lp) = lclAcc + accTr(lp)
