...
!!   modified to accrete percentage based on distance to sink by Josh Wall 2015
!!
!!***
...
!                        radius(pno_to_accrete) = sqrt(distx**2.0 + disty**2.0 + distz**2.0)
#ifdef ALFL_MSCALAR
                        ! in case we are using the Alfven limiter
                        alfl_dens = solnData(ALFL_MSCALAR,ip,jp,kp) * solnData(idens,ip,jp,kp)
                        
                        if (radius(pno_to_accrete) .lt. 0.4*accretion_radius_comoving) then
                        
                            mass = 0.85*mass
                            solnData(idens,ip,jp,kp) = (density_thresh_comoving + 0.15*(comovingCellDens - density_thresh_comoving) + alfl_dens)
                            
                        else if ((radius(pno_to_accrete) .ge. 0.4*accretion_radius_comoving) &
                                 .and. (radius(pno_to_accrete) .lt. 0.8*accretion_radius_comoving)) then
                                 
                            mass = 0.55*mass
                            solnData(idens,ip,jp,kp) = (density_thresh_comoving + 0.45*(comovingCellDens - density_thresh_comoving) + alfl_dens)
                            
                        else
                        
                            mass = 0.15*mass
                            solnData(idens,ip,jp,kp) = (density_thresh_comoving + 0.85*(comovingCellDens - density_thresh_comoving) + alfl_dens)
                        ...
                        if (radius(pno_to_accrete) .lt. 0.4*accretion_radius_comoving) then
                            ...
                            mass = 0.85*mass
                            solnData(idens,ip,jp,kp) = 0.15*(comovingCellDens - density_thresh_comoving) + density_thresh_comoving
                            
                        else if ((radius(pno_to_accrete) .ge. 0.4*accretion_radius_comoving) &
                                 .and. (radius(pno_to_accrete) .lt. 0.8*accretion_radius_comoving)) then
                                 
                            mass = 0.55*mass
                            solnData(idens,ip,jp,kp) = 0.45*(comovingCellDens - density_thresh_comoving) + density_thresh_comoving
                        
                        else
                        
                            mass = 0.15*mass
                            solnData(idens,ip,jp,kp) = 0.85*(comovingCellDens - density_thresh_comoving) + density_thresh_comoving
      ...
      if(affected_block) then
         ...
         ! I'm confused... we just calculated the new density
         ! can't we calculate the pres / ener from dens / temp?
