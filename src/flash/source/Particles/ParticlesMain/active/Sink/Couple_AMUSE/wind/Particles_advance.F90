...
subroutine Particles_advance (dtOld,dtNew)
  ...
  use Particles_interface, ONLY: Particles_sinkMoveParticles, Particles_sinkSortParticles, &
                                 ...
                                 Particles_sinkCreateAccrete, Particles_wind
  implicit none
...
!!------------------------------------------------------------------------------
  ...
  do kk=1,pt_numLocal
     write(*,*)'#1 local particle ',kk,'In proc ',int(particles(PROC_PART_PROP,kk)),'on blk=',int(particles(BLK_PART_PROP,kk))
...
#ifdef DEBUG_VPARTICLES
  do kk=1,pt_numLocal
     write(*,*)'#2 local particle ',kk,'In proc ',int(particles(PROC_PART_PROP,kk)),'on blk=',int(particles(BLK_PART_PROP,kk))
     write(*,*)'Xpos= ',particles(POSX_PART_PROP,kk),'Ypos=',particles(POSY_PART_PROP,kk)
  enddo
#endif 
...
!     case(RUNGEKUTTASTATIC) 
!        call pt_advanceRKstatic(dtOld,dtNew,particles(:,p_begin:p_end),&
!             p_count, i)
!     case(RUNGEKUTTATHERMO) 
!        call pt_advanceRKthermo(dtOld,dtNew,particles(:,p_begin:p_end),&
!             p_count, i)
!     case(RUNGEKUTTA) 
...
!             p_count, i)
!	 case(MONTECARLO)
!	    call pt_advanceMonteCarlo(dtOld,dtNew,particles(:,p_begin:p_end),&
!	         p_count, i)
!     end select
...
#ifdef DEBUG_VPARTICLES
  do kk=1,pt_numLocal
     write(*,*)'#3 local particle ',kk,'In proc ',int(particles(PROC_PART_PROP,kk)),'on blk=',int(particles(BLK_PART_PROP,kk))
     write(*,*)'Xpos= ',particles(POSX_PART_PROP,kk),'Ypos=',particles(POSY_PART_PROP,kk)
  enddo
#endif 
...
#ifdef DEBUG_VPARTICLES
  do kk=1,pt_numLocal
     write(*,*)'#4 local particle ',kk,'In proc ',int(particles(PROC_PART_PROP,kk)),'on blk=',int(particles(BLK_PART_PROP,kk))
     write(*,*)'Xpos= ',particles(POSX_PART_PROP,kk),'Ypos=',particles(POSY_PART_PROP,kk)
  enddo
#endif 
  
#ifdef AMUSE_STARS
  call stochastic(dtNew)
#else
...
#ifdef DEBUG_VPARTICLES
  do kk=1,pt_numLocal
     write(*,*)'#5 local particle ',kk,'In proc ',int(particles(PROC_PART_PROP,kk)),'on blk=',int(particles(BLK_PART_PROP,kk))
     write(*,*)'Xpos= ',particles(POSX_PART_PROP,kk),'Ypos=',particles(POSY_PART_PROP,kk)
  enddo
#endif   
...
#ifdef DEBUG_VPARTICLES
  do kk=1,pt_numLocal
     write(*,*)'#6 local particle ',kk,'In proc ',int(particles(PROC_PART_PROP,kk)),'on blk=',int(particles(BLK_PART_PROP,kk))
     write(*,*)'Xpos= ',particles(POSX_PART_PROP,kk),'Ypos=',particles(POSY_PART_PROP,kk)
  enddo
#endif 
...
#ifdef DEBUG_VPARTICLES
  do kk=1,pt_numLocal
     write(*,*)'#7 local particle ',kk,'In proc ',int(particles(PROC_PART_PROP,kk)),'on blk=',int(particles(BLK_PART_PROP,kk))
     write(*,*)'Xpos= ',particles(POSX_PART_PROP,kk),'Ypos=',particles(POSY_PART_PROP,kk)
  enddo
#endif 
...
! inject some wind
! Mass loss rate equation from Dale 2013
! 28 solar masses = 5.569618e34 gr
! 1e-6 solar masses/yr = 6.305286e19 gr/sec

!starMass = 30 * 1.989e33 ! 30 solar masses
!dm = ( 0.3 * exp(starMass / 5.569618e34 ) - 0.3 ) * 6.305286e19 * dtNew



!print *, "Calling Particles_wind with starMass = ", starMass, " dm = ", dm, " dtNew = ", dtNew

!call Particles_wind(starMass, dm, 0.0,0.0,0.0, dtNew)


  ! inject stellar winds
  !call Particles_sinkWind(dtNew)
  call Particles_wind(dtNew)
...
! just does star particle creation, this means they are created at the end 
! of the advancement, they are integrated with one of the standard advancement
! schemes, e.g. Leapfrog in the case block above in this routine
  call Particles_starsAdvance(dtNew)
