...
!!
!!   This version assumes that the internal energy was calculated with
!!   gamma = 1.0001 and so fixes the total energy check. It also includes
!!   a "fix" by Andrea Gatto that always accretes gas onto the sink to which
!!   it is most bound, no matter how close it is to another sink. - JW
!!
!!***
                                ...
                                do j1 = -ngc_sink_creation_check, ngc_sink_creation_check
                                   do i1 = -ngc_sink_creation_check, ngc_sink_creation_check
                                      ...
                                      if (i1**2 + j1**2 + k1**2 .le. ngc_sink_creation_check_radius_sqr) then
                                         ...
                                         !etherm_in_vol = etherm_in_vol + 0.6667 &
                                         etherm_in_vol = etherm_in_vol + 1.5 &
                                         & * solnData(TEMP_VAR,ii1,jj1,kk1) & 
                                         & * solnData(idens,ii1,jj1,kk1) &
                                         & * boltz / mH
