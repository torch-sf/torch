!!****if* source/Particles/ParticlesMain/active/AMUSE/Particles_massRemoval
!!
!! NAME
!!
!!  Particles_massRemoval
!!
!! SYNOPSIS
!!
!!  call Particles_massRemoval(real, intent(INOUT) :: mass_acc, xloc, yloc, zloc, velx, vely, velz)
!!
!! DESCRIPTION
!!
!!  Frankenstein constructed from Particles_sinkCreateAccrete
!!  This takes the gas over the critical density out of a cell and
!!  returns it to the calling routine. Does the sink particle checks
!!  to make sure the gas is collapsing and should be removed.
!!
!! ARGUMENTS
!!
!!   What you'd think the names mean.
!!
!! NOTES
!!
!!   Constructed from the body parts of Particles_sinkCreateAccrete by Josh Wall 2015
!!
!!***
...
!  call pt_sinkGatherGlobal(gather_propinds_pos, gather_nprops_pos)
  ...
  do lb = 1, blockCount
        ...
        if (refineLevel /= lrefine_max) then ! Are we at max refinement?
            ! If not, go to next block. Do not pass Go, do not collect $200.00!
            cycle
        end if
...
!                    end do
                    ...
                    if (create_sink) then
                       ...
                       if (create_sink) then
                            ...
                            call Grid_getPointData(blockID, SCRATCH_CTR, &
                                 SFTR_SCRATCH_CENTER_VAR, INTERIOR, [ip,jp,kp], &
                                 sf_timer)
                            
                            if (sf_timer == 0.0) then
                            call Grid_putPointData(blockID, SCRATCH_CTR, &
                                 SFTR_SCRATCH_CENTER_VAR, INTERIOR, [ip,jp,kp], &
                                 dt)
                            print*, "Starting timer on cell ", [ip,jp,kp], " at ", dt
                            
                            else if (sf_timer < 9.4608e12) then ! Less than 300k years
                            sf_timer = dt + sf_timer
                            call Grid_putPointData(blockID, SCRATCH_CTR, &
                                 SFTR_SCRATCH_CENTER_VAR, INTERIOR, [ip,jp,kp], &
                                 sf_timer)
                            print*, "Cell ", [ip,jp,kp], "timer is at ", sf_timer
                            cycle
                            
                            else
                            
                            
                            print*, "Actually about to take mass from this cell!"
                            print*, "Cell timer is at ", sf_timer
                            ncell = ncell + 1 ! # of cells mass is taken from
                            ! Gather the data to pass back to AMUSE.
                            mass_acc(ncell) = (comovingCellDens - density_thresh_comoving) * dVol
                            xloc(ncell) = xc(ip)
                            yloc(ncell) = yc(jp)
                            zloc(ncell) = zc(kp)
                            velx(ncell) = solnData(ivelx,ip,jp,kp)
                            vely(ncell) = solnData(ively,ip,jp,kp)
                            velz(ncell) = solnData(ivelz,ip,jp,kp)
#ifdef ALFL_MSCALAR
                            ! in case we are using the Alfven limiter
...
#else
                            solnData(idens,ip,jp,kp) = density_thresh_comoving ! Set the proper density (less the mass we removed)
#endif
!                            affected_block = .true.
