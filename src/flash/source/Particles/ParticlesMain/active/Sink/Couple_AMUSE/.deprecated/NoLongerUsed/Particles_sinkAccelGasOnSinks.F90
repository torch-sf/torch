...
! This shouldn't be called when we use the AMUSE coupling. -JW
  print*, "[Particles_sinkAccelGasOnSinks]: Who be calling me?"
  return

  if (localnpf .eq. 0) return

  if (Debug .and. dr_globalMe .eq. MASTER_PE) print *, 'Particles_sinkAccelGasOnSinks: entering.'
  ...
  if (Debug) then
      ...
      do p = 1, localnpf
        force_sum_x = force_sum_x + particles_global(MASS_PART_PROP,p)*particles_global(accxProp,p)
        force_sum_y = force_sum_y + particles_global(MASS_PART_PROP,p)*particles_global(accyProp,p)
        force_sum_z = force_sum_z + particles_global(MASS_PART_PROP,p)*particles_global(acczProp,p)
      end do
