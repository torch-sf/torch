...
!!   modified to kick the gas over a timestep dt - Josh Wall, 2015
!!
!!***

subroutine Particles_sinkKickGas(part_mass, part_x, part_y, part_z, nparts, &
                                 blockCount, blockList, dt, accelVars)
...
include "Flash_mpi.h"
 ...
 real, dimension(nparts), intent(IN) :: part_mass, part_x, part_y, part_z
    ...
    do ii = blkLimitsGC(LOW,IAXIS)+1, blkLimitsGC(HIGH,IAXIS)-1
       do jj = blkLimitsGC(LOW,JAXIS)+1, blkLimitsGC(HIGH,JAXIS)-1
          do kk = blkLimitsGC(LOW,KAXIS)+1, blkLimitsGC(HIGH,KAXIS)-1
               ...
               !do p = 1, localnpf
			   ! Loop over the passed positions and masses.
			   do p=1, nparts

                  !mass = particles_global(MASS_PART_PROP,p)
                  mass = part_mass(p)
                  !print*, "Kick grid reports mass to be =", mass

                  dx = x(ii) - part_x(p) ! particles_global(POSX_PART_PROP,p)
                  dy = y(jj) - part_y(p) ! particles_global(POSY_PART_PROP,p)
                  dz = z(kk) - part_z(p) ! particles_global(POSZ_PART_PROP,p)
               ...
               ! Changed here to give the gas a velocity kick for the AMUSE
               ! gravity bridge. - JW

               ! x-acceleration:
               solnData(VELX_VAR,ii,jj,kk) = solnData(VELX_VAR,ii,jj,kk) + dt*paccx*prefactor

               ! y-acceleration:
               solnData(VELY_VAR,ii,jj,kk) = solnData(VELY_VAR,ii,jj,kk) + dt*paccy*prefactor

               ! z-acceleration:
               solnData(VELZ_VAR,ii,jj,kk) = solnData(VELZ_VAR,ii,jj,kk) + dt*paccz*prefactor
