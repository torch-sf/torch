...
#include "GridParticles.h"
  ...
  ! My variables - JW
  real    :: prob, rand_num, gaus_rad, coeff, coeff_sum
  real    :: returned_mass, t_ff, avg_dens
  real    :: pxcm_in_vol, pycm_in_vol, pzcm_in_vol
  real, save :: min_mass, sfe_local, sigma
  integer :: n_sinks, num_cells, save_tags(maxsinks)
  integer :: pt_saved, ind_rad_sq
...
#define get_ppe(arg1) ((arg1) - get_pno(arg1)*65536)
  ...
  if (first_call) then
    ...
    call RuntimeParameters_get("sfe", sfe_local)
    call RuntimeParameters_get("gaussian_sigma", sigma)
    ...
    !sigma = sigma*delta_at_lrefmax
    sigma = 0.5*accretion_radius
    
    if (MyPE .eq. MasterPE) print*, "Min mass is", min_mass
    if (MyPE .eq. MasterPE) print*, "SFE is ", sfe_local
    if (MyPE .eq. MasterPE) print*, "Gaussian sigma*delta_at_lrefmax is ", sigma
              ...
              do ip = blkLimits(LOW,IAXIS), blkLimits(HIGH,IAXIS)
                 ...
                 if (comovingCellDens .gt. density_thresh_comoving) then
                    ...
                    if (create_sink) then
                       ...
                       ! Sum the mass and momentum from the surrounding cells
                       ! that we want to make a particle from using the
                       ! percentages from gaussian profile of gas removal. - JW
                       if (create_sink) then
                          ...
                          do k1 = -ngc_sink_creation_check-2, ngc_sink_creation_check+2
                            do j1 = -ngc_sink_creation_check-2, ngc_sink_creation_check+2
                              do i1 = -ngc_sink_creation_check-2, ngc_sink_creation_check+2

                                ii1 = ip+i1
                                jj1 = jp+j1
                                kk1 = kp+k1

                                !ind_rad_sq = i1**2 + j1**2 + k1**2
                                gaus_rad = (xc(ip)-xc(ii1))**2.0 + (yc(jp)-yc(jj1))**2.0 + &
                                              (zc(kp)-zc(kk1))**2.0
                                
                                !if (ind_rad_sq .LE. 9) then
                                
                                !print*, "Gaus rad", gaus_rad
                                !print*, "Acc rad", accretion_radius_comoving**2.0
                                
                                if (gaus_rad .le. accretion_radius_comoving**2.0) then
                                ! Calculate the distance from the center
                                ! cell for the gaussian function.
                                
                                ! Note we need to make sure this norms
                                ! to 1 for all the cells coeff summed up.
                                   
                                  coeff = exp(gaus_rad/(2.0*(sigma)**2.0))
                                  ...
                                  ! to remove mass, vel etc from cells that
...
!                                  if ((ii1 .lt. blkLimits(IAXIS,LOW)) then
!                                    off_block  = .true. 
!                                    neghDir(1) = LEFT_EDGE
!                                    neghInd(1) = ii1 + NXB - NGUARD
!                                  else if (ii1 .gt. blkLimits(IAXIS,HIGH))
!                                    off_block = .true.
!                                    neghDir(1) = RIGHT_EDGE
!                                    neghInd(1) = ii1 - NXB + NGUARD
!                                  else
!                                    neghDir(1) = CENTER
!                                    neghInd(1) = ii1
!                                  end if
!                                  if ((jj1 .lt. blkLimits(JAXIS,LOW)) then
!                                    off_block  = .true. 
!                                    neghDir(2) = LEFT_EDGE
!                                    neghInd(2) = jj1 + NYB - NGUARD
!                                  else if (jj1 .gt. blkLimits(JAXIS,HIGH))
!                                    off_block = .true.
!                                    neghDir(2) = RIGHT_EDGE
!                                    neghInd(2) = jj1 - NYB + NGUARD
!                                  else
!                                    neghDir(2) = CENTER
!                                    neghInd(2) = jj1
!                                  end if
!                                  if ((kk1 .lt. blkLimits(KAXIS,LOW)) then
!                                    off_block  = .true. 
!                                    neghDir(3) = LEFT_EDGE
!                                    neghInd(3) = kk1 + NZB - NGUARD
!                                  else if (kk1 .gt. blkLimits(KAXIS,HIGH))
!                                    off_block = .true.
!                                    neghDir(3) = RIGHT_EDGE
!                                    neghInd(3) = kk1 - NZB + NGUARD
!                                  else
!                                    neghDir(3) = CENTER
!                                    neghInd(3) = kk1
...
!                                      call Grid_getBlkNeighBlkIDFromPos(lb, [xc(ii1), yc(jj1), zc(kk1)], neghDir, neghBlk, neghProc)
!                                      print*, "We are currently on proc ", dr_MyPe, " and block ", lb
!                                      print*, "This cell is on proc ", neghProc, " and block ", neghBlk
!                                      print*, "and its i, j, k are ", neghInd 
!                                  end if
#ifdef ALFL_MSCALAR
                                  ! in case we are using the Alfven limiter
                                  gas_dens = (1.0-solnData(ALFL_MSCALAR,ii1,jj1,kk1)) * &
                                  ...
                                  avg_dens = avg_dens + gas_dens ! Avg dens of the whole gas in cell
                                  
                                  !etherm_in_vol = etherm_in_vol + solnData(ieint,ii1,jj1,kk1)*gas_dens 

                                  !pxcm_in_vol = pxcm_in_vol + solnData(ivelx,ii1,jj1,kk1)*gas_dens
                                  !pycm_in_vol = pycm_in_vol + solnData(ively,ii1,jj1,kk1)*gas_dens
                                  !pzcm_in_vol = pzcm_in_vol + solnData(ivelz,ii1,jj1,kk1)*gas_dens

                                  mass_in_vol = mass_in_vol + coeff*gas_dens ! Mass which we'd actually remove
                                  
                                  end if ! If in index radius
                          ...
                          print*, "Coeff sum is ", coeff_sum
                          print*, "Local sfe is ", sfe_local
                          
                          avg_dens = avg_dens / num_cells
                          t_ff     = sqrt(3.0*PI/(32.0*newton*avg_dens))
                          mass_in_vol = mass_in_vol * dVol / coeff_sum / num_cells
                        
                          prob = sfe_local*(mass_in_vol / (min_mass*1.989e33)) * dt / t_ff
                        
                          print*, "Avg density with coeff is ", avg_dens
                          print*, "Free fall time is ", t_ff
                          print*, "Mass with coeff in accr radius is ", mass_in_vol / 1.989e33
                          print*, "Number of cells dens gained from is ", num_cells
                          !print*, "Coeff is ", coeff
                          ...
                          if ((prob / rand_num) .lt. 1.0) &
                            create_sink = .false.
                            
                       end if
                       
                       ! Now lets sample the IMF for a mass to create.
                       if (create_sink) then
                         
                         call IMF_prob((mass/1.989e33), min_mass, returned_mass, prob)
                         
                         print*, "Probability from IMF is ", prob, " to create &
                                  a star of mass ", returned_mass
...
!                                ind_rad = i1**2 + j1**2 + k1**2
                                
!                                if (ind_rad .LE. 4) then
!                                ! Calculate the distance from the center
!                                ! cell for the gaussian function.
!                                  gaus_dist = (xc(ip)-xc(i1))**2.0 + (yc(ip)-yc(i1))**2.0 + &
!                                              (zc(ip)-zc(i1))**2.0 
!                                  coeff = c1*exp(gaus_dist/(2*(sigma)**2.0)
...
!                                      print*, "Hey, one of these cells is not on this block."
!                                      call Grid_getBlkNeighBlkIDFromPos(lb, [xc(ii1), yc(jj1), zc(kk1)], neghDir, neghBlk, neghProc)
!                                      if (neghProc .ne. dr_MyPe) off_proc = .true. 
!                                      print*, "We are currently on proc ", dr_MyPe, " and block ", lb
!                                      print*, "This cell is on proc ", neghProc, " and block ", neghBlk
!                                      print*, "and its i, j, k are ", neghInd 
!                                  end if
                                  
!                                  if (off_proc .or. off_block) then
                                  
!                                    Call Grid_putPointData
!#ifdef ALFL_MSCALAR
!                                  ! in case we are using the Alfven limiter
!                                  gas_dens = coeff*(1.0-solnData(ALFL_MSCALAR,ii1,jj1,kk1)) * &
!                                              solnData(idens,ii1,jj1,kk1)
                                              
!                                  alfl_dens = solnData(ALFL_MSCALAR,ip,jp,kp) * solnData(idens,ip,jp,kp)
                                              
!                                  solnData(idens, ii1, jj1, kk1) = solnData(idens, ii1, jj1, kk1) - gas_dens
!                                  solnData(ALFL_MSCALAR,ii1,jj1,kk1) = ALFL_MSCALAR,ii1,jj1,kk1) / solnData(idens, ii1, jj1, kk1)
!#else
!                                  gas_dens = coeff*solnData(idens,ii1,jj1,kk1)
!                                  solnData(idens, ii1, jj1, kk1) = solnData(idens, ii1, jj1, kk1) - gas_dens
!#endif
!                                  !etherm_in_vol = etherm_in_vol + solnData(ieint,ii1,jj1,kk1)*gas_dens 
...
!                        end if
                       ...
                       if (create_sink) then
                          ...
                          n_sinks = n_sinks + 1
                          save_tags(n_sinks) = int(particles_local(iptag,pno))
  ...
  print*, "Number of sinks created is ", n_sinks
  
  ! For us, if no sinks were created just return. - JW
  
  if (n_sinks .eq. 0) return
              ...
              !if (comovingCellDens .gt. density_thresh_comoving) then
                 ...
                 if (np_found .gt. 0) then
                    ...
                    do npf = 1, np_found
                       ...
                       do pt_saved=1, n_sinks
                       
                       if (int(particles_local(iptag,pno)) .eq. save_tags(pt_saved)) then
                       ...
                       ! Calculate the exponental coeff to use for mass. -JW
                       coeff = 1.0/(sigma*sqrt(2.0*PI))*exp(radius(npf)/(2*(sigma)**2.0))
                       ! Make sure we normalize by the correct amount
                       ! found in the loop above, and also factor in the
                       ! local SFE (20-30% from Krumholz et al 2011). - JW
                       ! This also makes the hopeful assumption that the
                       ! number of cells the density is taken from is the same
                       ! as the last sink made above (such that coeff_sum is the
                       ! same). Note this should be true since sinks can only
                       ! form at the max refinement, but it should be checked. -JW
                       mass = comovingCellDens*coeff*dVol / coeff_sum * sfe_local

                       !egrav_Gas = -Newton*2.0 * PI / 3.0 * density_thresh_comoving * &
                       !   & (accretion_radius_comoving**2 - radius(npf)**2)*mass
                       ! This should use the left over gas density not the dens_thres gas density.
                       egrav_Gas = -Newton*2.0 * PI / 3.0 * (comovingCellDens - mass/dVol)  * &
                          & (accretion_radius_comoving**2 - radius(npf)**2)*mass
                       ...
                       end if ! If its a saved tag we just created.
                       end do ! Loop over saved tags.
                    ...
                    do npf = 1, np_found
                      ...
                      do pt_saved=1, n_sinks ! Was this sink just created? -JW
                    ...
                    if (pno_to_accrete .gt. 0) then
                        ...
                        ! Calculate the coefficient for exponential accretion.
                        gaus_rad = distx**2.0 + disty**2.0 + distz*2.0
                        coeff = 1.0/(sigma*sqrt(2.0*PI))*exp(gaus_rad/(2*(sigma)**2.0)) / coeff_sum * sfe_local
                        
#ifdef ALFL_MSCALAR
                        ! in case we are using the Alfven limiter
                        alfl_dens = solnData(ALFL_MSCALAR,ip,jp,kp) * solnData(idens,ip,jp,kp)
                        !solnData(idens,ip,jp,kp) = density_thresh_comoving + alfl_dens
                        solnData(idens,ip,jp,kp) = (solnData(idens,ip,jp,kp)-solnData(idens,ip,jp,kp)*coeff) + alfl_dens 
                        solnData(ALFL_MSCALAR,ip,jp,kp) = alfl_dens / solnData(idens,ip,jp,kp)
#else
                        !solnData(idens,ip,jp,kp) = density_thresh_comoving
                        solnData(idens,ip,jp,kp) = solnData(idens,ip,jp,kp)-solnData(idens,ip,jp,kp)*coeff
#endif
                        affected_block = .true.
