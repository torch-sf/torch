...
! TESTING with energyinj
use GridInject_interface, ONLY : GridInject_wind
...
#ifdef debug2
  if (dr_globalMe .eq. 0) &
    print*, "Calling GridInject_wind with mass, dt, dmdt, vwind, bgdy =", mass, dt, dmdt(p)/solarMass*yr, v_wind(p), bgdy(p)
!    print*, "Calling inject direct with mass, dt, dmdt, vwind, bgdy =", mass, dt, dmdt(p)/solarMass*yr, v_wind(p), bgdy(p)
#endif
  
  call GridInject_wind(x(p), y(p), z(p), mass, v_wind(p), twind, dt, bgdy(p))
