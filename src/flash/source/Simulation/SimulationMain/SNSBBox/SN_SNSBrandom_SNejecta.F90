...
!	integer, parameter	  :: nms = 50! 'nms' is the number of shells used to compute r_exp where M~60 M_sun
	...
	! iterators
	integer :: imass, i, j, k, l, m, im_exp, isum, im_MC, im_shell
	...
	real	:: r_exp, v_exp, outshf, r_shell
	real	:: xx, yy, zz, xb, yb, zb
	real	:: x0heat, y0heat, z0heat

	! parameters for contact discontinuity injection
	real	:: masstot, qheatratio, qheatshell, r_shl, restMass, rho_shl, massshell
...
! fixed for singleSN
					...
					y0h(i) = 0d0
					z0h(i) = 0d0
				else if (he_nSN .eq. 2) then
! half a box length away
					x0h(i) = 0.60d0*(gr_imax - gr_imin)+gr_imin
					y0h(i) = 0.60d0*(gr_jmax - gr_jmin)+gr_jmin
					z0h(i) = 0.60d0*(gr_kmax - gr_kmin)+gr_kmin
				endif
...
! calculate shell index, shell density and radius 
		  im_shell = im_exp
		  do im_shell = im_exp, 1, -1
		    r_shl = (outshf**(im_shell-1))*he_r_init
! find first shell that is outside of shell 
		    if(r_shl .le. (r_exp - sn_shellThicky)) then        
!					if(he_meshMe == MASTER_PE) then
...
! let's see if this works subtract mass shells to get shell mass

      rho_shl = mass_sum_tot(im_shell)-mass_sum_tot(im_exp)
      if(rho_shl .eq. 0d0 .or. im_shell .eq. im_exp) then
				if(he_meshMe == MASTER_PE) then
						print*, im_shell, im_exp
						print*,'he dead jim'
				endif
			endif

! get totMass*ratio into shell volume, so a density of rho_shl
      masstot = mass_sum_tot(im_exp)
!      rho_shl = mass_sum_tot(im_exp)*sn_MassRatio/(vol_sum_tot(im_exp) - vol_sum_tot(im_shell))

			massshell = sn_MassRatio*(vol_sum_tot(im_exp) - vol_sum_tot(im_shell)) / vol_sum_tot(im_shell)
			massshell = massshell/(1d0+massshell)*masstot
      rho_shl = massshell/(vol_sum_tot(im_exp) - vol_sum_tot(im_shell))

!      restMass = mass_sum_tot(im_exp)*(1d0-sn_MassRatio)
      restMass = masstot-massshell!mass_sum_tot(im_exp)*(1d0-sn_MassRatio)
! get the rest into the interior
			rho_avg = restMass/vol_sum_tot(im_shell)

! V shell / V total
			qheatratio	 = ( vol_sum_tot(im_exp) - vol_sum_tot(im_shell) )/vol_sum_tot(im_exp)
			qheat        = dt*vol_sum_tot(im_shell)
			qheatshell   = dt*( vol_sum_tot(im_exp) - vol_sum_tot(im_shell) )

			qheat        = he_exp_energy*(1d0-qheatratio)/qheat
			qheatshell   = he_exp_energy*(qheatratio)/qheatshell

			if(he_meshMe == MASTER_PE) then
	      print*,'shell index',im_shell, im_exp
 		    print*,'radii',r_shl, r_exp
 		    print*,'densities',rho_shl, rho_avg
				print*,'totmass, restmass, massshell', masstot, restMass, massshell
				print*,'volumes',vol_sum_tot(im_exp), vol_sum_tot(im_shell), (vol_sum_tot(im_exp) - vol_sum_tot(im_shell))
				print*,'mass from volume',rho_avg*vol_sum_tot(im_shell), rho_shl*(vol_sum_tot(im_exp) - vol_sum_tot(im_shell))
  	    print*,'heat ratio',qheatratio, 1d0-qheatratio
			endif
...
!						MCradius = r_shl*0.5
					  ...
					  do im_MC = he_nms, 1, -1
					    ...
					    if(MCradius .le. (r_exp - sn_MCTracerShellDis)  ) then
!					    if(MCradius .le. (r_shl*0.5)  ) then
								if(he_meshMe == MASTER_PE) then
									print*,'injection and expl. radius, shell index'
									print*, MCradius, r_exp, im_MC
								endif
...
!  			if(he_veltracer) then 
!					ncount = nzones_sum_loc(im_exp)*he_tracerPerZone**3
!      	else 
!					ncount = nzones_sum_loc(im_exp)*he_tracerPerZone
!				endif
...
!						if((he_TracerPerSN - tagcounter - ncount*nzones_sum_loc(im_exp)) .eq. leftovers) then
!							injectLeftovers = .true.
!						endif
					endif
					he_oldStartTag = pt_startTagNumber + leftovers
...
! SB output, always worked on first
			if(nSBw .gt. 0) then
				call WriteSNFeedback(he_nSN-nSBw+1, 3, dr_nstep, time, x0heat, y0heat, z0heat, r_exp, masstot, ntracer)
				nSBw = nSBw - 1
...
! could have multiple calls fix
				call WriteSNFeedback(he_nSN-nSN, SNType(l), dr_nstep, time, x0heat, y0heat, z0heat, r_exp, masstot, ntracer)
			endif
