...
subroutine Particles_sinkAccelGasOnSinksAndSinksOnGas(accelProps,accelVars)
  ...
  ! user modification: StratBox static grav acceleration on sinks
  use Simulation_data, ONLY : sim_aParm1, sim_aParm2, sim_aParm3, sim_aParm4, sim_withStaticGrav
...
#include "Particles.h"
  ...
  real               :: zHeight, abszHeight, rhoNFW  ! user modification: StratBox static grav acceleration on sinks
  real               :: exc, eyc, ezc
...
!!$     call Driver_abortFlash('Particles_sinkAccelGasOnSinksAndSinksOnGas: ERROR in index of particle property.')
  ...
  ! Add static z-acceleration on sinks -- StratBox modification
  if (updateSinkProps .and. sim_withStaticGrav) then
     ! Loop over all particles, local and global
     do p = 1, localnpf

        zHeight = particles_global(POSZ_PART_PROP,p)
        abszHeight = abs(zHeight)

        ! TODO: sim_NFWh not ported over from Juan's Stratboxes.
        ! -AT, 2019 May 09
    !if (abszHeight  .lt. sim_NFWh) then
         ! gravitational acceleration due to stellar mass distribution
       az = - (sim_aParm1*zHeight/dsqrt(zHeight*zHeight + sim_aParm3*sim_aParm3)) - &
    ...
    !   az = - 4./3d0 * sim_GravConst * PI * rhoNFW * zheight
    !endif

        particles_global(acczProp,p) = particles_global(acczProp,p) + az
