!!****if* source/Simulation/SimulationMain/StratBox/Grid_markRefineDerefine
!!
!! NAME
...
!!
!! This is a _fork_ of the FLASH4.5 source file
!!
!!    Grid/GridMain/paramesh/Grid_markRefineDerefine.F90
!!
!! modified to (1) ensure refinement on stellar winds from active (non-sink)
!! particles, and (2) apply a z-stratifieid nested refinement.  It can be
!! further modified to implement zoom refinement on specific regions,
!! particles, and such.
!! The special refinement prescriptions were taken from:
!!
!!    flash4.5-rad/source/Particles/ParticlesMain/active/Sink/Couple_AMUSE/Grid_markRefineDerefine.F90
!!    flash4.2.2-rad/source/Simulation/SimulationMain/StratBox_TracerParticles/Grid_markRefineDerefine.F90
!!
!! created by Joshua Wall (JW), Christian Baczynski (CB), and Juan Camilo
!! Ibanez-Mejia (JCIM); merged by Aaron Tran (AT).
!! -- AT, 2019 March 28
!!
!!***
...
!!$  use physicaldata, ONLY : force_consistency
  ...
  use Simulation_data, only: sim_zrefcut1, sim_zrefcut2, sim_zrefcut3, &
                             sim_zrefcut4, sim_zrefcut5, sim_zrefcut6, &
                             sim_zrefcut7, sim_useNestedRef, sim_targetRef
...
#include "Flash.h"
  ...
  ! CB added to do geometric refinement
  integer, parameter :: specsSize = 7
  real,dimension(specsSize) :: specs
...
!!$  force_consistency = .TRUE.
  ...
  ! CB added to do geometric refinement
  if (sim_useNestedRef) then
     specs = (/ sim_zrefcut1, sim_zrefcut2, sim_zrefcut3, sim_zrefcut4, sim_zrefcut5, sim_zrefcut6, sim_zrefcut7/)
     call Grid_markRefineSpecialized(NESTEDZ, specsSize, specs, sim_targetRef)
  endif
