...
#ifdef FLASH_MPI2
      !USE mpi_f08 ! This for future use with FLASH_MPI3! - JW
      USE MPI
      Implicit None
#else
      Implicit None
...
#ifdef FLASH_MPI2
      ! Define stride integers of MPI_ADDRESS_KIND
      ! for MPI-3 standard compatability. - JW
      Integer                        :: stride1
      Integer(KIND=MPI_ADDRESS_KIND) :: stride2, stride3
#else
...
!-----This code added for reordering script
      ...
      stride1 = udim_tot(1) ! Note stride1 isn't in bytes, but in elements!
      stride2 = udim_tot(1)*udim_tot(2)*nbytes
      stride3 = udim_tot(1)*udim_tot(2)*udim_tot(3)*nbytes
...
!-----DEFINE BLOCK INTERIOR
      Call MPI_TYPE_VECTOR (udim(2),                                   & 
                            ...
                            ierr)
#ifdef FLASH_MPI2
      Call MPI_TYPE_CREATE_HVECTOR (udim(3),                           & 
                             1,                                        &
                             stride2,           & 
                             type1,                                    & 
                             type2,                                    & 
                             ierr)
      Call MPI_TYPE_CREATE_HVECTOR (udim(4),                           &
                             1,                                        &
                             stride3, &
                             type2,                                    &
                             type3,                                    &
...
!-----treat unk_n
      ...
      Do While (repeat.And.nit<130)
         ...
         End If
         if (nm2 .ne. 0 .AND. nit .ge. 100 .AND. mod(nit,5) == 0) Then
            ! After ~100 iterations, give the underlying communications several
            ! chances to settle down from long a delay that may afflict some
            ! asynchronous messages. - KW
            if (mype == 0) print *,'* sleeping for one second in mpi_amr_redist_blk'
            call dr_sleep(1)
         end if
         
         nit = nit + 1
         
      End Do  ! End Do While (repeat.And.nit<130)
      
      If (nm2.ne.0.And.nit>=130) Then
         If (mype == 0) Then
