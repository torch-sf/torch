...
!#endif
  ...
  ! At the end of an evolve step, the potential is correct
  ! for the current density field. No need to recalculate. - JW
...
!  if (grav_unjunkPden) then
     ...
     do lb = 1, blockCount
        ...
        ! Also store the correct potential in newPotVar
        solnVec(newPotVar,:,:,:) = solnVec(newPotVar,:,:,:) - solnVec(GPOT_VAR,:,:,:)

        call Grid_releaseBlkPtr(blocklist(lb), solnVec)
     enddo
     call Grid_notifySolnDataUpdate( (/density, newPotVar/) )
#endif
!  end if
#endif
