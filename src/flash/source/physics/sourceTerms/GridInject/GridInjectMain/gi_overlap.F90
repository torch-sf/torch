...
subroutine gi_overlap(ishp, rad, center, cell_bot, cell_top, nsteps, overlap_vol)
  ...
  real(dp)                    :: inside_weight, a, d, ratio
...
! Now sample these to see if they are inside the object.
  ...
  do i=1, nsteps
    do j=1, nsteps
      do k=1, nsteps
        ...
        if (ratio .le. 1.0_dp) then
            inside_weight = inside_weight + 1.0_dp
        else if (ratio .le. sqrt(2.0_dp)) then
            inside_weight = inside_weight + 3.0_dp*(rad/sqrt(3.0_dp))/r-2.0_dp
        else if (r .le. rad) then
            a = (3.0_dp/sqrt(2.0_dp)-2.0_dp)/(sqrt(3.0_dp)-sqrt(2.0_dp))**2.0_dp
            inside_weight = inside_weight + a*(sqrt(3.0_dp)-ratio)**2
        end if
  ...
  overlap_vol = inside_weight / real(nsteps**3.0_dp)
