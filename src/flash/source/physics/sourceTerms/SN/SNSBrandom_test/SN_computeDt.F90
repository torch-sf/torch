...
#include "constants.h"
  ...
  use SN_data, only: he_meshMe, he_newDt, he_useSN 
...
!	  print*,'ugh',he_newDt,he_heat_flag
  if(he_useSN) then  
	  !cgs 1000 yr are 3.1556926e10 seconds 
!	  print*,'ugh',he_newDt
	  dt_check = he_newDt !usually 3.1556926e10 
	  !TODO put in block or zone location of SN blast
	  dt_minloc(1) = 1
	  dt_minloc(2) = 1
	  dt_minloc(3) = 1
	  dt_minloc(4) = 1
	  dt_minloc(5) = he_meshMe
  endif
