...
!  first, check if it's time for a field supernova of Type I
		if (t0heat < tstar1) then
          ...
          z0h(i) = he_kmax
          do while (z0h(i) .ge. he_kmax .or. z0h(i) .le. he_kmin)
            z0h(i) = grndSN()*2.0 - 1.0   !-1 to 1
            z0h(i) = abs(z0h(i))/z0h(i) * log(abs(z0h(i))) * he_hstar1 ! inverse transform, sgn(z)*ln(|z|)/hstar1
          end do
				else
          z0h(i) = grndSN()*(he_kmax-he_kmin) + he_kmin
				endif
		...
		if (t0heat < tstar2) then
          ...
          do while (z0h(i) .ge. he_kmax .or. z0h(i) .le. he_kmin)
            z0h(i) = grndSN()*2.0 - 1.0   !-1 to 1
            z0h(i) = abs(z0h(i))/z0h(i) * log(abs(z0h(i))) * he_hstar2 ! inverse transform, sgn(z)*ln(|z|)/hstar2
          end do
				else
          z0h(i) = grndSN()*(he_kmax-he_kmin) + he_kmin
				endif
		...
		do l = 1, num_exp
			...
			if(nSBr .gt. 0) then
				do i = sbIndex+1, sb_nSN
			  	...
			  	enddo ! end SB loop.
			else ! if exp is not SB but SN then
				x0heat = x0h(l)
				y0heat = y0h(l)
				z0heat = z0h(l)
			endif
...
!							argm   = argm + min( (zz-z0heat)**2, (zz-(z0heat+he_kmax-he_kmin))**2, (zz-(z0heat-he_kmax+he_kmin))**2 )
							...
							do imass = 1, he_nms
								...
								if (argm <= ((outshf**(imass-1))*he_r_init)*((outshf**(imass-1))*he_r_init)) then
...
sloop: do isum = 1, he_nms
       ...
       enddo sloop
			...
			im_exp = he_nms
      		...
      		if(he_meshMe == MASTER_PE) then 
				  print*,'##did not find enough mass:',mass_sum_tot(he_nms)/he_mejc
					...
					! this gives only little information, compare to phenHeat output
					write(sn_funit_evol,'(2(1X,I16),6(1X,ES16.9),5(1X,I16))') &
					dr_nStep, SNtag, dr_simTime,0d0,0d0,0d0, &
					0d0,mass_sum_tot(he_nms)/he_mejc,i,j,k,blockID,he_meshMe
					close(sn_funit_evol)
		...
		qheat   = dt*vol_sum_tot(im_exp) ! [cm^3 s ]
		qheat   = he_exp_energy/qheat 	 ! [erg / (cm^3 s) ]
...
!								tranheat = qheat
! smooth temperature internal energy and density
!								tmp      = tmp*rho/rho_avg
!								ei       = ei*rho/rho_avg
!								rho      = rho_avg
...
! call pointwise eos, don't have to call eos on each block and each zone
! eos is called for the smoothing of the explosion region, not actual energy injection
! energy injection is done in Heat.F90 in /phenHeat
!								pointLimit(LOW,IAXIS) = i
!								pointLimit(LOW,JAXIS) = j
!								pointLimit(LOW,KAXIS) = k
!								pointLimit(HIGH,IAXIS) = i
!								pointLimit(HIGH,JAXIS) = j
!								pointLimit(HIGH,KAXIS) = k
...
! check if radiation transport is included by looking for ionisation rate field
				...
				!  crank changed state variables through EOS,  again only changed blockIDs should be called
				!  
