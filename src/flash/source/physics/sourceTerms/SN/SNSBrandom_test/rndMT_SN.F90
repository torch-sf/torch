!!!!!!!!!!!!!!!!!! DUPLICATE of mtmod to keep a second stream of random numbers from main stream
!!!!!!!!!!!!!!!!!! otherwise calls outside of SN would break lockstepping
...
!                              a default initial seed is used
    ...
    if(y .lt. 0) then
      grndSN=(dble(y)+2.0d0**32)/(2.0d0**32-1.0d0)
    else
      grndSN=dble(y)/(2.0d0**32-1.0d0)
    endif
...
!NOTE: This subroutine APPENDS to the end of the file "fname". CB not anymore it don't
...
!            position='APPEND')
       open(unit=17,file=trim(fname),status='replace',form='UNFORMATTED',action='write')
       write(17)mti
       write(17)mt
...
!            position='APPEND')
       open(unit=17,file=trim(fname),status='replace',form='FORMATTED',action='write')
       write(17,*)mti
       write(17,*)mt
...
!         state information will be read in formatted form.
  subroutine mtgetf( fname, forma )
    ...
    select case (forma)
      case('u','U')
       open(unit=17,file=trim(fname),status='OLD',form='UNFORMATTED',action='read')
       read(17)mti
       read(17)mt

      case default
       open(unit=17,file=trim(fname),status='OLD',form='FORMATTED',action='read')
       read(17,*)mti
       read(17,*)mt
