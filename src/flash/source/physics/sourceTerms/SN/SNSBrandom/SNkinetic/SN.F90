...
!			  z0heat = DNINT(z0heat/gridspace)*gridspace
					...
					zoneindex = floor((x0heat) / gridspace(1))
!					x0heat = gridspace(1)*(zoneindex + 0.5d0)
					x0heat = gridspace(1)*(zoneindex)
					zoneindex = floor((y0heat) / gridspace(2))
!					y0heat = gridspace(2)*(zoneindex + 0.5d0)
					y0heat = gridspace(2)*(zoneindex)
					zoneindex = floor((z0heat) / gridspace(3))
!					z0heat = gridspace(3)*(zoneindex + 0.5d0)
					z0heat = gridspace(3)*(zoneindex)
       endif
			endif
...
! energy injection per mass
       ...
       enddo
			...
			else if(he_injVol) then

			do isum = 1, he_nms
				if ((vol_sum_tot(isum)/4.18879020479)**(1d0/3d0) .ge. he_maxradius) then
					im_exp = isum
			  	exit
				endif
       enddo
...
! maybe reroll to a different location?
    	...
    	if(he_meshMe == MASTER_PE) then 
			...
			v_exp		= vol_sum_tot(im_exp)
	   	mass    = rho_avg*vol_sum_tot(im_exp)
			vel_exp = sqrt(2d0*he_exp_energy/mass)

			if(he_meshMe == MASTER_PE) then
				print*, 'total kinetic energy injected',vel_exp*vel_exp*rho_avg*v_exp*0.5d0
			endif
...
!  get zone variables
								gridspace = gr_delta(1:MDIM,lrefine(blockID))
								rho       = rho_avg
								! zonemass  = gridspace(1)*gridspace(2)*gridspace(3)*rho
								! vel_exp   = zonemass
								! fraction of total mass
								! zonemass  = zonemass/mass
								! vel_exp   = sqrt(vel_exp*zonemass)
								!vel_exp   = sqrt(2d0*he_exp_energy/mass)
								! added to current velocities
								velx = solnData(VELX_VAR,i,j,k) + vel_exp*(xx-x0heat)/r_shell
								vely = solnData(VELY_VAR,i,j,k) + vel_exp*(yy-y0heat)/r_shell
								velz = solnData(VELZ_VAR,i,j,k) + vel_exp*(zz-z0heat)/r_shell
