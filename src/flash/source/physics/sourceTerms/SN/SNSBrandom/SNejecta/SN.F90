...
!!
!!=====
!! TODO tie MCTracerShelldis to shell from SN, like half the radius of the shell should be ejecta radius
!!=====
!! ARGUMENTS
!!
...
!  calculate new step to land before explosion add some slop
		he_newDt = 0.5*(tnext1 - time - 0.9*he_SNminstep)
...
! V shell / V total
			...
			if(he_meshMe == MASTER_PE) then
	      print*,'shell index, exp index',im_shell, im_exp
      	print*,'shell, interior densities',rho_shl, rho_avg
				print*,'tot., int., shell mass', masstot, restMass, massshell
				print*,'tot., int., shell volumes',vol_sum_tot(im_exp), vol_sum_tot(im_shell), (vol_sum_tot(im_exp) - vol_sum_tot(im_shell))
				print*,'shell, int. energy ratio',qheatratio, 1d0-qheatratio
			endif
