...
! tracers
	...
	call RuntimeParameters_get('shellThicky', sn_shellThicky)
	call RuntimeParameters_get('MassRatio', sn_MassRatio)
...
! Errrybody should know these
  ...
  if(he_useSNTracer) then
    ...
    if (he_meshMe .eq. MASTER_PE) then
      print*,'mapping SB particle properties to tracer particle properties' 
      print*,'mass prop-> dens'
      print*,'nsn prop -> temp'
      print*,'tsn prop -> SN number'
      print*,'sbid prop -> free'
    endif
  endif
