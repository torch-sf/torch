...
subroutine SB_init()
	...
	use SN_data, ONLY: he_meshMe, he_nSN
...
! SN rates and numbers
	...
	call RuntimeParameters_get('SB_cluster_peak', sb_peak_position)

! first index is 0, minimum value 
	sb_edge(1) = 0.e0

! defaults are nsnmin 7, nsnmax 40
...
! read through all SN
			do
				...
				if(c2 .gt. dr_simTime) then
					sb_nSN   = c0-1
					exit
		    endif
    	enddo

! if checkpoint file time equals last SB time
			if(sb_nSN .eq. 0 .and. c0 .gt. 0) then
				sb_nSN = c0
			endif 
    ...
    if (he_meshMe .eq. MASTER_PE) then
        print*,'Restarting from SB number', sb_nSN
    endif

		close(funit_evol)
	endif

	allocate(sbPOS(1:4,1:sb_SBmax))
	allocate(sbPOSbuff(1:4,1:sb_SBmax)) 
