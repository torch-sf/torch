!!****f* source/physics/sourceTerms/SN/Simple/SN_computeDt
!!
!! NAME
!!  
!!  SN_computeDt
...
!! SYNOPSIS
!! 
!!  SN_computeDt ( integer(IN) : blockID, 
!!                   
!!                  real(IN):  x(:), 
!!                  real(IN): dx(:), 
!!                  real(IN): uxgrid(:),
...
!! DESCRIPTION
!!
!!  Computes the timestep limiter for SN source term solver.
!!
!! ARGUMENTS
!!
...
subroutine SN_computeDt (blockID, &
                              x, dx, uxgrid, &
                              y, dy, uygrid, &
                              z, dz, uzgrid, &
...
#include "constants.h"
  ...
  use SN_data, only: sn_meshMe, sn_newDt, sn_useSN 
  ...
  if(sn_useSN) then  
    dt_check = sn_newDt !usually 3.1556926e10 
    !TODO put in block or zone location of SN blast
    dt_minloc(1) = 1
    dt_minloc(2) = 1
    dt_minloc(3) = 1
    dt_minloc(4) = 1
    dt_minloc(5) = sn_meshMe
  endif
