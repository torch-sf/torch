...
! Global variable(s):
      INTEGER IT, NT      
      REAL, DIMENSION(5,6) :: eD_rosseland, eD_planck
      REAL, DIMENSION(71,71) :: eG_rosseland, eG_planck
      REAL opac_floor
      !REAL*8 eD, eG
      !DIMENSION eD(5,6)
      !DIMENSION eG(71,71)
...
! Initialization of all necessary data for a chosen dust model:
      ...
      call init_d(.TRUE.,model,top,shape,eD_rosseland)
      call init_d(.FALSE.,model,top,shape,eD_planck)
! Initialization of data for the gas model:
      !call init_g(ross,eG)
      call init_g(.TRUE.,eG_rosseland)
      call init_g(.FALSE.,eG_planck)

! Read floor opacity value
      call RuntimeParameters_get("opac_floor",opac_floor)
!   
...
subroutine getOpacity(temp,rho,opac)
   ...
   ! This was in our version originally so I'm adding it back in to try to fix a problem
   ...
   !
   IF(ross) THEN
     CALL getOpacity_rosseland(temp,rho,opac)
   ELSE
     CALL getOpacity_planck(temp,rho,opac)
   END IF
   !
...
subroutine getOpacity_rosseland(temp,rho,opac)
   implicit none
   real, intent(in)    :: temp,rho
   real, intent(out) :: opac
   real :: temp_,rho_
   temp_ = MIN(MAX(temp,5.),9999.)
   rho_ = MIN(MAX(rho,2.E-18),1.0E-7)
   CALL cop(eD_rosseland,eG_rosseland,rho_,temp_,opac)
   opac = max(opac,opac_floor)
end subroutine getOpacity_rosseland

subroutine getOpacity_planck(temp,rho,opac)
   implicit none
   real, intent(in)    :: temp,rho
   real, intent(out) :: opac
   real :: temp_,rho_
   temp_ = MIN(MAX(temp,5.),9999.)
   rho_ = MIN(MAX(rho,2.E-18),1.0E-7)
   CALL cop(eD_planck,eG_planck,rho_,temp_,opac)
   opac = MAX(opac,opac_floor)
end subroutine getOpacity_planck

subroutine getOpacity_star(pno,temp,rho,opac)
#ifdef SINK_PART_TYPE
   use Particles_sinkData, ONLY : particles_global
   use Cool_data, ONLY : cl_sigmaSB
#endif
   implicit none
   integer, intent(in) :: pno
   real, intent(in)    :: temp, rho
   real, intent(out) :: opac
   real :: temp_,rho_,luminosity,radius,Teff
!#ifdef SINK_PART_TYPE
!   luminosity = particles_global(LUMINOSITY_PART_PROP,pno)
!   radius = particles_global(STELLAR_RADIUS_PART_PROP,pno)
!
!   IF(radius.gt.0.) THEN
!     ! Get the effective temperature
!     Teff = (luminosity/(4.*PI*radius**2*cl_sigmaSB))**0.25
!   ELSE ! Only accretion luminosity, choose background temperature
!     Teff = temp
!   END IF
!#else
!   Teff = temp
!#endif
!   temp_ = MIN(MAX(Teff,5.),9999.)
   temp_ = MIN(MAX(temp,5.),9999.)
   rho_ = MIN(MAX(rho,2.E-18),1.0E-7)
   CALL cop(eD_planck,eG_planck,rho_,temp_,opac)
end subroutine getOpacity_star
!
!............................................................................. 
...
! Fit coefficients for Planck mean opacity (extinction)  
      DATA  eP(1,6),eP(1,5),eP(1,4),eP(1,3),eP(1,2),eP(1,1),& 
       ...
       if(dr_globalMe.eq.MASTER_PE) print *,'Dust model: "normal" silicates; homogeneous spheres'
...
! Fit coefficients for Planck mean opacity (extinction)
      DATA  eP(1,6),eP(1,5),eP(1,4),eP(1,3),eP(1,2),eP(1,1), &
       ...
       if(dr_globalMe.eq.MASTER_PE) print *,'Dust model: "normal" silicates; homogeneous dust', &
                                                ' aggregates'
       if(dr_globalMe.eq.MASTER_PE) print*,'-------------------------------------------------'
...
! Fit coefficients for Planck mean opacity (extinction)  
      DATA  eP(1,6),eP(1,5),eP(1,4),eP(1,3),eP(1,2),eP(1,1), & 
       ...
       if(dr_globalMe.eq.MASTER_PE) print *,'Dust model: "normal" silicates; composite dust', &
                                                ' aggregates'
       if(dr_globalMe.eq.MASTER_PE) print*,'----------------------------------------------------'
...
! Fit coefficients for Planck mean opacity (extinction)  
      DATA  eP(1,6),eP(1,5),eP(1,4),eP(1,3),eP(1,2),eP(1,1), &
       ...
       if(dr_globalMe.eq.MASTER_PE) print *,'Dust model: "iron-rich" silicates; homogeneous spheres'
       if(dr_globalMe.eq.MASTER_PE) print*,'----------------------------------------------------'
...
! Fit coefficients for Planck mean opacity (extinction)
      DATA  eP(1,6),eP(1,5),eP(1,4),eP(1,3),eP(1,2),eP(1,1),& 
       ...
       if(dr_globalMe.eq.MASTER_PE) print *,'Dust model: "iron-rich" silicates; homogeneous dust', &
                                                 ' aggregates'
       if(dr_globalMe.eq.MASTER_PE) print*,'----------------------------------------------------'
...
! Fit coefficients for Planck mean opacity (extinction)  
      DATA  eP(1,6),eP(1,5),eP(1,4),eP(1,3),eP(1,2),eP(1,1),&  
       ...
       if(dr_globalMe.eq.MASTER_PE) print *,'Dust model: "iron-rich" silicates; composite dust', &
                                                  ' aggregates'
       if(dr_globalMe.eq.MASTER_PE) print*,'----------------------------------------------------'
...
! Fit coefficients for Planck mean opacity (extinction)  
      DATA  eP(1,6),eP(1,5),eP(1,4),eP(1,3),eP(1,2),eP(1,1),& 
       ...
       if(dr_globalMe.eq.MASTER_PE) print *,'Dust model: "iron-poor" silicates; homogeneous spheres'
       if(dr_globalMe.eq.MASTER_PE) print*,'----------------------------------------------------'
...
! Fit coefficients for Planck mean opacity (extinction)  
      DATA  eP(1,6),eP(1,5),eP(1,4),eP(1,3),eP(1,2),eP(1,1),& 
       ...
       if(dr_globalMe.eq.MASTER_PE) print *,'Dust model: "iron-poor" silicates; homogeneous dust', &
                                                   ' aggregates'
       if(dr_globalMe.eq.MASTER_PE) print*,'----------------------------------------------------'
...
! Fit coefficients for Planck mean opacity (extinction)
      DATA  eP(1,6),eP(1,5),eP(1,4),eP(1,3),eP(1,2),eP(1,1),& 
       ...
       if(dr_globalMe.eq.MASTER_PE) PRINT *,'Dust model: "iron-poor" silicates; composite', &
              ' aggregates'
       if(dr_globalMe.eq.MASTER_PE) print*,'----------------------------------------------------'
...
! Fit coefficients for Planck mean opacity (extinction)
      DATA  eP(1,6),eP(1,5),eP(1,4),eP(1,3),eP(1,2),eP(1,1),&  
       ...
       if(dr_globalMe.eq.MASTER_PE) PRINT *,'Dust model: "iron-rich" silicates; porous', &
              ' composite spherical particles'
       if(dr_globalMe.eq.MASTER_PE) print*,'----------------------------------------------------'
! Set up output parameters:
       DO i = 1,5
...
! Fit coefficients for Planck mean opacity (extinction)
      DATA  eP(1,6),eP(1,5),eP(1,4),eP(1,3),eP(1,2),eP(1,1),& 
       ...
       if(dr_globalMe.eq.MASTER_PE) PRINT *,'Dust model: "normal" silicates; composite', &
              ' spherical particles'
       if(dr_globalMe.eq.MASTER_PE) print*,'----------------------------------------------------'
! Set up output parameters:
       DO i = 1,5
...
! Fit coefficients for Planck mean opacity (extinction)
      DATA  eP(1,6),eP(1,5),eP(1,4),eP(1,3),eP(1,2),eP(1,1),& 
       ...
       if(dr_globalMe.eq.MASTER_PE) PRINT *,'Dust model: "normal" silicates; porous', &
              ' composite spheres'
       if(dr_globalMe.eq.MASTER_PE) print*,'----------------------------------------------------'
! Set up output parameters:
       DO i = 1,5
...
! Fit coefficients for Planck mean opacity (extinction)
      DATA  eP(1,6),eP(1,5),eP(1,4),eP(1,3),eP(1,2),eP(1,1),&  
       ...
       if(dr_globalMe.eq.MASTER_PE) PRINT *,'Dust model: "iron-rich" silicates; composite', &
              ' spheres'
       if(dr_globalMe.eq.MASTER_PE) print*,'----------------------------------------------------'
! Set up output parameters:
       DO i = 1,5
...
! Fit coefficients for Planck mean opacity (extinction)
      DATA  eP(1,6),eP(1,5),eP(1,4),eP(1,3),eP(1,2),eP(1,1),& 
       ...
       if(dr_globalMe.eq.MASTER_PE) PRINT *,'Dust model: "iron-rich" silicates; compact', &
              ' 5-layered spheres'
       if(dr_globalMe.eq.MASTER_PE) print*,'----------------------------------------------------'
! Set up output parameters:
       DO i = 1,5
...
! Fit coefficients for Planck mean opacity (extinction)
      DATA  eP(1,6),eP(1,5),eP(1,4),eP(1,3),eP(1,2),eP(1,1),&  
       ...
       if(dr_globalMe.eq.MASTER_PE) PRINT *,'Dust model: "iron-poor" silicates; porous', &
              ' 5-layered spheres'
       if(dr_globalMe.eq.MASTER_PE) print*,'----------------------------------------------------'
! Set up output parameters:
       DO i = 1,5
